...
test('it should be able to reset password within 2 hours after token generation', async ({
  assert,
  client,
}) => {
  const email = 'fkinvest@gmail.com';
  ...
  await Database.table('tokens')
    ...
    .update(
      'created_at',
      format(subMinutes(new Date(), 119), 'yyyy-MM-dd HH:mm:ss')
    );
...
test('it should block reset password if token was generated more than 2 hours ago', async ({
  assert,
  client,
}) => {
  const email = 'fkinvest@gmail.com';
  ...
  await Database.table('tokens')
    ...
    .update(
      'created_at',
      format(subMinutes(new Date(), 120), 'yyyy-MM-dd HH:mm:ss')
    );
